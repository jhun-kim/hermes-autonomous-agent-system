...
7. macOS에서는 cmux가 기본 작업 공간입니다. cmux가 없으면 설치를 안내하고 `cmux version`으로 확인한 뒤 작업을 시작하세요.
8. Windows에서는 cmux를 요구하지 말고 Windows Terminal/PowerShell에서 Codex CLI가 보이는 상태로 작업하게 해주세요.
9. Hermes Discord gateway 연결은 dry-run 검증이 통과하기 전에는 live 모드로 실행하지 마세요.
10. 설치가 끝나면 어떤 명령으로 정상 설치를 확인했는지 알려주세요.
...
- macOS: Spotlight에서 `Terminal` 검색 또는 LLM에게 "Terminal.app을 열어줘"라고 요청. 작업 시작 전 `cmux version`으로 cmux를 확인하고, 없으면 LLM이 설치를 안내하게 하세요.
- Windows: 시작 메뉴에서 `Windows Terminal` 또는 `PowerShell` 실행. cmux를 요구하지 말고 `codex --version` 확인 후 터미널에서 Codex CLI가 보이게 작업하세요.
- Linux: 배포판의 `Terminal` 앱 실행. cmux가 있으면 사용하고, 없으면 보이는 터미널 Codex fallback을 사용하세요.
